...
export async function createExpedienteFolder({ caseNumber, nombre, apellido1, files = [], tramite = '' }) {
  ...
  if (pdfName) {
    try {
      ...
      if (!sourceRes.ok) {
        console.error(`PDF no encontrado en Formatos oficiales: ${pdfName} — ${sourceRes.status}`);
      } else {
        const sourceItem = await sourceRes.json();
        const sourceId = sourceItem.id;

        const destFolderRes = await gFetch(token, `/sites/${siteId}/drive/root:/${encodePath(`${expedientePath}/02_Formularios_Oficiales`)}`);
        if (!destFolderRes.ok) {
          console.error(`Carpeta destino no encontrada: ${destFolderRes.status}`);
        } else {
          const destFolder = await destFolderRes.json();

          const copyRes = await gFetch(token, `/sites/${siteId}/drive/items/${sourceId}/copy`, {
            method: 'POST',
            body: JSON.stringify({
              parentReference: { driveId: destFolder.parentReference.driveId, id: destFolder.id },
              name: `${caseNumber}_${pdfName}`,
            }),
          });

          if (copyRes.ok || copyRes.status === 202) {
            console.log(`PDF copiado correctamente: ${pdfName}`);
          } else {
            console.error(`Error copiando PDF: ${copyRes.status} ${await copyRes.text()}`);
          }
        }
      }
    } catch(e) {
      console.error(`Error en copia PDF: ${e.message}`);
    }
  }
