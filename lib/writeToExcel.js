...
async function ensureFolder(token, siteId, folderPath) {
  ...
  for (const name of parts) {
    ...
    const res = await gFetch(token, parentEndpoint, {
      method: 'POST',
...
export async function createExpedienteFolder({ caseNumber, nombre, apellido1 }) {
  ...
  await Promise.all(
    SUBFOLDERS.map(sub =>
      gFetch(token, `/sites/${siteId}/drive/root:/${encodePath(expedientePath)}:/children`, {
        method: 'POST',
        body: JSON.stringify({ name: sub, folder: {}, '@microsoft.graph.conflictBehavior': 'ignore' }),
      }),
    ),
  );
