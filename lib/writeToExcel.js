...
export async function createExpedienteFolder({ caseNumber, nombre, apellido1, files = [], tramite = '', data = {} }) {
  const token  = await getGraphToken();
  const siteId = await getSiteId(token);
  ...
  // Rellenar PDF del trámite y subir a 02_Formularios_Oficiales
  const pdfName = TRAMITE_PDF_MAP[tramite];
  if (pdfName) {
    try {
      const sourceRes = await gFetch(token, `/sites/${siteId}/drive/root:/${encodePath(`Formatos oficiales/${pdfName}`)}`);
      if (!sourceRes.ok) {
        console.error(`PDF no encontrado: ${pdfName} — ${sourceRes.status}`);
      } else {
        const sourceItem = await sourceRes.json();

        const downloadRes = await fetch(sourceItem['@microsoft.graph.downloadUrl']);
        const pdfBytes = await downloadRes.arrayBuffer();

        const filledPdfBytes = await fillPdf(pdfBytes, data);

        const destPath = encodePath(`${expedientePath}/02_Formularios_Oficiales/${caseNumber}_${pdfName}`);
        const uploadRes = await fetch(`${GRAPH}/sites/${siteId}/drive/root:/${destPath}:/content`, {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        if (uploadRes.ok) {
          console.log(`PDF rellenado y subido: ${caseNumber}_${pdfName}`);
        } else {
          console.error(`Error subiendo PDF rellenado: ${uploadRes.status} ${await uploadRes.text()}`);
        }
      }
    } catch(e) {
      console.error(`Error en relleno PDF: ${e.message}`);
    }
  }
