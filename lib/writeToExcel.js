...
export async function createExpedienteFolder({ caseNumber, nombre, apellido1, files = [], tramite = '' }) {
  ...
  if (pdfName) {
    try {
      ...
      const copyRes = await gFetch(token, `/sites/${siteId}/drive/root:/${sourcePath}:/copy`, {
        method: 'POST',
        body: JSON.stringify({
          parentReference: {
            siteId: siteId,
      ...
      if (!copyRes.ok) {
        ...
        console.error(`Error copiando PDF: ${copyRes.status} ${errText}`);
      } else {
        console.log(`PDF copiado correctamente: ${pdfName} → status ${copyRes.status}`);
        const location = copyRes.headers.get('Location');
        if (location) console.log(`Monitor copia en: ${location}`);
      }
    } catch (e) {
