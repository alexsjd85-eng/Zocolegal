...
// Encode each segment of a slash-separated path for use in Graph API URLs.
...
// Creates each folder level in path if it doesn't already exist.
// Uses conflictBehavior=ignore so existing folders are not treated as errors.
async function ensureFolder(token, siteId, folderPath) {
  ...
  for (const name of parts) {
    ...
    const res = await gFetch(token, `${parentEndpoint}?@microsoft.graph.conflictBehavior=ignore`, {
      method: 'POST',
      body: JSON.stringify({ name, folder: {} }),
    });

    // 409 = already exists (some tenants return this instead of 200 with ignore)
    if (!res.ok && res.status !== 409) {
      throw new Error(`ensureFolder "${name}" ${res.status}: ${await res.text()}`);
    }
...
async function getOrCreateFile(token, siteId) {
  // Ensure parent folder exists before checking/creating the file.
...
export async function createExpedienteFolder({ caseNumber, nombre, apellido1 }) {
  const token  = await getGraphToken();
  const siteId = await getSiteId(token);

  // Sanitize client name: remove chars forbidden in SharePoint folder names.
  ...
  // Create the full path including the case folder.
  await ensureFolder(token, siteId, expedientePath);

  // Create the 5 subfolders in parallel.
  await Promise.all(
    SUBFOLDERS.map(sub =>
      gFetch(token, `/sites/${siteId}/drive/root:/${encodePath(expedientePath)}:/children?@microsoft.graph.conflictBehavior=ignore`, {
        method: 'POST',
